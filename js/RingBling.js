...
RingBling.prototype.init = function()
{
	...
	this.geo = this.RingGeometry(this.radius, this.thickness, this.radialSegments, this.tubularSegments, Math.PI*2, this.extrude);
...
RingBling.prototype.updateGeometry = function(that)
{
	...
	that.geo = that.RingGeometry(that.radius, that.thickness, that.radialSegments, that.tubularSegments, Math.PI*2, that.extrude);
	this.remove(that.mesh);
...
RingBling.prototype.RingGeometry = function(radius, thickness, radialSegments, tubularSegments, arc, extrude)
{
	...
	geo.vertices.forEach(function(item, index) {
		...
		if (zero.distanceTo(item2d) < radius-thickness) {
			item.setLength(1+radius);
		}
	});
